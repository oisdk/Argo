...
prefix func >|<Element>(rhs: List<Element>)(_ lhs: Element) -> List<Element> {
  return lhs >| rhs
}

postfix func >|<Element>(lhs: Element)(_ rhs: List<Element>) -> List<Element> {
  return lhs >| rhs
}
...
extension List {
  public func map<T>(f: Element -> T) -> List<T> {
    ...
    case let (x,xs)?: return f(x) >| xs().map(f)
    }
  }
}
```
...
extension List {
  public func map<T>(transform: Element -> T) -> List<T> {
    guard case let (x,xs)? = self else { return .None }
...
extension List: GeneratorType, LazySequenceType {
  ...
  public mutating func next() -> Element? {
    guard case let (x,xs)? = self else { return nil }
    self = xs()
    return x
  }
}
...
public func ==<E:Equatable>(lhs: List<E>, rhs: List<E>) -> Bool {
  switch (lhs,rhs) {
  case let ((x,xs)?,(y,ys)?): return x == y && xs() == ys()
...
extension List {
  ...
  public var count: Int {
    guard case let (_,t)? = self else { return 0 }
...
extension List {
  ...
  public func appended(@autoclosure(escaping) with: () -> Element) -> List<Element> {
    guard case let (x,xs)? = self else { return [with()] }
...
extension List {
  ...
  public func dropFirst(n: Int) -> List<Element> {
    switch (n,self) {
    case (0,_) ,(_,.None): return self
    case (_, let (_,tail)?): return tail().dropFirst(n - 1)
    }
  }
  
  private func divide(at: Int) -> (List<Element>, List<Element>) {
    switch (at, self) {
    case (0, _), (_, .None): return (.None, self)
    case (_, let (x,xs)?):
  ...
  private func dropLast(var ahead: [Element]) -> List<Element> {
    guard case let (x,xs)? = self else { return .None }
    ahead.append(x)
  ...
  public func dropLast(n: Int) -> List<Element> {
    ...
    return back.dropLast(Array(front))
  }
}
...
extension List {
  ...
  public func prefixWhile(isElement: Element -> Bool) -> List<Element> {
    switch self {
    case let (x,xs)? where isElement(x):
  ...
  public func dropWhile(@noescape isNotElement: Element -> Bool) -> List<Element> {
    switch self {
    case let (x,xs)? where isNotElement(x):
...
extension List {
  ...
  public func prefix(n: Int) -> List<Element> {
    switch (n, self) {
    case (0, _), (_, .None): return .None
    case let (_, (x,xs)?): return x >| xs().prefix(n - 1)
    }
  }
  ...
  public func suffix(n: Int) -> List<Element> {
    return zip(0..<n, reverse()).reduce(List.None) { $1.1 >| $0 }
  }
  
  private func divide(@noescape isSplit: Element throws -> Bool) rethrows -> (List<Element>, List<Element>) {
    guard case let (x,xs)? = self else { return (.None, .None) }
    if try isSplit(x) { return (.None, xs()) }
...
extension List {
  ...
  public mutating func removeFirst() -> Element {
    guard case let (x,xs)? = self else { fatalError("Cannot call removeFirst() on an empty List") }
    self = xs()
    return x
  }
  /**
...
extension List {
  private func rev(other: List<Element>) -> List<Element> {
    guard case let (x,xs)? = self else { return other }
...
extension List {
  ...
  public func reduce<T>(initial: T, @noescape combine: (accumulator: T, element: Element) -> T) -> T {
    guard case let (x,xs)? = self else { return initial }
